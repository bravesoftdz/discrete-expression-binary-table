...
procedure GetTable(Suggestion: ansiString);
  ...
  begin
    ...
    if length(tmp) = 2 then begin  //may be prefix "not" statement
      if (OpDetect(tmp[0]) = 12) or ((OpDetect(tmp[0]) = 10)) then begin
        IDXl := 1; IDXr := _length;
        result := suggestion;
      end;
    end else begin
  ...
  begin
    ...
    if length(chain) = 2 then begin //NOT X statement
			if (OpDetect(chain[0]) = 12) or ((OpDetect(chain[0]) = 10)) then
				op := $c
			else begin
...
begin
writeln('функции:');
writeln('  help - показать это сообщение');
writeln('  D+ / D- - включить/выключить тестовый вывод');
writeln('  exit - завершить приложение');
writeln('переменные: "a" - "z"');
writeln('константы: "0", "1" ');
writeln('приоритет операторов: "(", ")"');
